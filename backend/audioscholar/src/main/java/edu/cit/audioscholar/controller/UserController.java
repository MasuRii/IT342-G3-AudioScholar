...
public class UserController {
    ...
    public ResponseEntity<?> createUser(@Valid @RequestBody User user) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage() != null ? e.getMessage() : "Error creating user.");
    ...
    public ResponseEntity<?> getUserById(@PathVariable String userId) {
        try {
            ...
            return user != null ? ResponseEntity.ok(user) : ResponseEntity.notFound().build();
             ...
             return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage() != null ? e.getMessage() : "Error retrieving user data.");
        } catch (Exception e) {
    ...
    public ResponseEntity<?> updateUser(@PathVariable String userId, @Valid @RequestBody User user) {
        if (user.getUserId() == null || user.getUserId().isEmpty()) {
             ...
             return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("User ID in path does not match User ID in request body.");
        }
        ...
        try {
            ...
            return ResponseEntity.ok(updatedUser);
             ...
             return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage() != null ? e.getMessage() : "Error updating user data.");
        } catch (IllegalArgumentException e) {
    ...
    public ResponseEntity<?> findUserByEmail(@RequestParam(value = "email", required = true) String email) {
        ...
        try {
            ...
            if (user != null) {
             ...
             return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e.getMessage() != null ? e.getMessage() : "Error searching users.");
        } catch (Exception e) {
             logger.error("Unexpected error finding user by email [{}]: {}", email, e.getMessage(), e);
             return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("An unexpected error occurred while searching users.");
        }
    }
}