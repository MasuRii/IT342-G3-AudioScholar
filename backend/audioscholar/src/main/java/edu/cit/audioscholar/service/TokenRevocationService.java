    ...
    public boolean isTokenRevoked(String token) {
        try {
            ...
            if (revoked) {
                ...
                log.debug(
                        "Token with jti {} not found in denylist. Access allowed (pending other checks).",
                        jti);
            }
            return revoked;
            ...
            log.error("Unexpected error while checking token revocation status: {}", e.getMessage(),
                    e);
            return true;
        }
    }
