// components/RecordingData.jsx
import React, { useState, useEffect } from 'react';
import { useParams, Link, useNavigate } from 'react-router-dom';
import axios from 'axios';
import { API_BASE_URL } from '../services/authService'; // Ensure this path is correct
import ReactMarkdown from 'react-markdown'; // Import ReactMarkdown

// Icons (simple placeholders, consider using react-icons if available)
...
const RecordingData = () => {
  const { id } = useParams(); // This is the AudioMetadata ID from the URL/workaround
  const navigate = useNavigate();

  const [recordingData, setRecordingData] = useState(null); // Holds AudioMetadata
  ...
  // State for Recommendations (RE-ADDED)
  const [recommendationsData, setRecommendationsData] = useState([]);
  const [recommendationsLoading, setRecommendationsLoading] = useState(false);
  const [recommendationsError, setRecommendationsError] = useState(null);

  // Active Tab State
  const [activeTab, setActiveTab] = useState('summary'); // 'summary', 'transcript', 'notes'

  // Function to fetch specific recording data (using workaround)
  const fetchRecordingData = async () => {
    ...
    try {
      ...
      // Find using the metadata ID from the URL param
  ...
  // Fetch Summary (and potentially recommendations later) using the correct recordingId
  const fetchDetails = async (actualRecordingId) => {
    setSummaryLoading(true);
    setSummaryError(null);
    setRecommendationsLoading(true); // Re-added
    setRecommendationsError(null); // Re-added
    ...
    if (!token) {
      ...
      setRecommendationsLoading(false); // Re-added
      navigate('/signin');
      return;
    }
    ...
    try {
      ...
      // Handle Recommendations Response (RE-ADDED)
  ...
  // Fetch base recording data first
  useEffect(() => {
    if (id) { // id is the metadata ID from URL
      fetchRecordingData();
    }
  }, [id]);

  // Fetch summary and recommendations *after* recording data is available
  useEffect(() => {
    // Use the recordingId *from* the fetched metadata to fetch details
    if (recordingData?.recordingId) {
       fetchDetails(recordingData.recordingId);
    }
  }, [recordingData]); // Re-run when recordingData (metadata) changes
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F0F4F8]"> {/* Changed background */}

      {/* Header - Consistent with RecordingList/Uploading */}
              ...
              <div>
                ...
                {!summaryLoading && !summaryError && summaryData && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {/* Left Column (takes 2/3 width on md+) */}
                    <div className="md:col-span-2 space-y-6">
                       ...
                       {/* Display Formatted Summary using ReactMarkdown */}
                        <div>
                           <h3 className="font-semibold text-lg mb-2 text-gray-800">Summary Details</h3>
                           {/* Apply prose class for basic markdown styling (requires @tailwindcss/typography or custom styles) */}
                    ...
                    {/* Right Column (takes 1/3 width on md+) */}
                       ...
                       {summaryData.glossary && summaryData.glossary.length > 0 && (
                        ...
                        <div>
                            ...
                            <ul className="list-disc list-inside text-gray-700 space-y-1 text-sm">
                                {/* Render questions here if available */}
      ...
      {/* Footer - Consistent (Optional - remove if not needed in this view) */}
