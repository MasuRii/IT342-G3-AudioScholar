...
const SignIn = () => {
    ...
    // Optional: Redirect if user is already logged in via Firebase (client-side)
    // You might want to check for your *backend* JWT instead for persistence
    // useEffect(() => {
    //     const unsubscribe = onAuthStateChanged(auth, (user) => {
    //         if (user) {
    //            // Maybe check if backend token exists too?
    //            // navigate('/dashboard');
    //         }
    //     });
    ...
    const handleBackendVerification = async (idToken) => {
        ...
        try {
            ...
            // Call the service function that hits your /verify-firebase-token endpoint
            ...
            // Optional: Sign out from Firebase client-side if backend verification fails
            // auth.signOut();
        } finally {
            setLoading(false);
        }
            ...
            if (err.code) {
                switch (err.code) {
                    ...
                    case 'auth/too-many-requests':
                         ...
                         break;
                    default:
                         errorMessage = err.message; // Use the default Firebase error message
                }
            }
            setError(errorMessage);
            setLoading(false); // Ensure loading stops on Firebase error
        }
                ...
                <div className="max-w-4xl mx-auto grid md:grid-cols-2 rounded-lg shadow-xl overflow-hidden">
                    ...
                    <div className="bg-white p-8 md:p-10">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 mb-6">
                            <button
                                onClick={handleGoogleSignIn} // Use Google handler
                        ...
                        <form className="space-y-4" onSubmit={handleSubmit}>
                            ...
                            <div className="flex items-center justify-between text-sm">
                                <div className="flex items-center">
                                    {/* Note: Firebase handles session persistence differently.
                                         The 'Remember me' checkbox might not directly map
                                         unless you implement custom session duration logic */}
                                ...
                                {/* Link to your forgot password page (likely needs Firebase integration too) */}
