...
const UserProfileEdit = () => {
  ...
  // Add state to store initial fetched values for comparison
  const [initialFirstName, setInitialFirstName] = useState('');
  const [initialLastName, setInitialLastName] = useState('');

  // --> Add state for avatar file and preview <--
  const [avatarFile, setAvatarFile] = useState(null);
  const [avatarPreview, setAvatarPreview] = useState(null);
  const [currentProfileImageUrl, setCurrentProfileImageUrl] = useState(null); // Renamed from currentAvatarUrl
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        // Store initial names for comparison on save
        setInitialFirstName(userData.firstName || '');
        setInitialLastName(userData.lastName || '');
        setCurrentProfileImageUrl(userData.profileImageUrl); // <-- Use profileImageUrl
  ...
  // --> Add handler for avatar file selection <--
  const handleAvatarChange = (e) => {
    ...
    if (file) {
      ...
      const maxSize = 5 * 1024 * 1024; // 5MB (matching backend)
  ...
  const handleSaveChanges = async (e) => {
    ...
    let avatarUploaded = false; // <-- Track avatar upload status

    // --- 1. Update Profile Details (Name Only) --- 
    const profileDataChanged = 
        firstName !== initialFirstName || 
        lastName !== initialLastName;
        
    // Email is no longer editable via this form.
    if (profileDataChanged) {
        try {
            console.log('Attempting to update profile details (name only)...');
            const updatePayload = { firstName, lastName }; // Only send firstName and lastName
    ...
    // --- 2. Upload Avatar --- 
    // Only attempt upload if details/password didn't fail and a new file is selected
    if (!errorsOccurred && avatarFile) {
        try {
            ...
            avatarUploaded = true;
            // Assuming the avatar endpoint *also* returns profileImageUrl
            setCurrentProfileImageUrl(response.data.profileImageUrl); // <-- Use profileImageUrl
            setAvatarFile(null); // Clear selected file state
            setAvatarPreview(null); // Clear preview
            document.getElementById('avatarInput').value = null; // Clear file input visually
    ...
    if (newPassword || confirmPassword) {
        if (newPassword !== confirmPassword) {
            ...
            errorsOccurred = true;
        } else if (newPassword.length < 6) { // Basic length check (mirroring backend)
             setError('New password must be at least 6 characters long.');
             errorsOccurred = true;
        } else {
            // Only proceed if details update didn't already fail (or if details weren't changed)
            if (!errorsOccurred) {
                try {
    ...
    if (!errorsOccurred && (detailsUpdated || passwordChanged || avatarUploaded)) { // <-- Added avatarUploaded
      let message = [];
      if (detailsUpdated) message.push('Profile details updated.');
      if (passwordChanged) message.push('Password changed.');
      if (avatarUploaded) message.push('Avatar updated.'); // <-- Added avatar message
      setSuccessMessage(message.join(' ') + ' Successfully!');
       // Optionally navigate back after a short delay
       // setTimeout(() => navigate('/profile'), 2000);
    } else if (!errorsOccurred && !detailsUpdated && !passwordChanged && !avatarUploaded) { // <-- Added avatarUploaded
  ...
  const handleCancel = () => {
    navigate('/profile'); // Navigate back to the profile view
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-100">
      {/* Header (Consistent with UserProfile) */}
      <header className="bg-[#1A365D] shadow-sm py-4">
         <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
           <Link to="/dashboard" className="text-2xl font-bold text-white">AudioScholar</Link>
            {/* Replaced Cancel button with Back to Dashboard Link */}
           <Link
              to="/dashboard" // Link to the dashboard
              className="flex items-center text-gray-300 hover:text-white transition-colors py-2 px-3 rounded hover:bg-white hover:bg-opacity-10 text-sm" // Style similar to other headers
            >
              {/* Dashboard Icon */}
              ...
              <span className="sm:hidden">Dashboard</span> {/* Short text for small screens */}
            </Link>
         </div>
       </header>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-lg mx-auto bg-white p-8 md:p-10 rounded-lg shadow-xl">
            ...
            <form onSubmit={handleSaveChanges} className="space-y-5">
              {/* --> Add Avatar Preview and Upload Section <-- */}
              <div className="flex flex-col items-center space-y-4 mb-4">
                 <label className="block text-sm font-medium text-gray-700">Profile Picture</label>
                 <img
                   src={avatarPreview || currentProfileImageUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(initialFirstName || '')} ${encodeURIComponent(initialLastName || '')}&background=1A365D&color=fff&size=128`} // Use profileImageUrl
                   alt="Avatar Preview"
